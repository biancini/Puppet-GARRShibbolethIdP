...
function getProvince() {
        ...
        $fileName = "/usr/share/phpldapadmin/lib/ripartizioni_regioni_province.csv";
        foreach (file($fileName) as $currow) {
                $vals = explode(';', $currow);

                if (sizeof($vals) > 13) {
                        $provincia = $vals[13];

                        if (strpos($provincia, '/') > 0) {
                                $provincia = substr($provincia, 0, strpos($provincia, '/'));
                        }

                        $province[strtolower($provincia)] = $provincia;
                }
        }

        ksort($province);
        $province["estero"] = "Estero";
        return $province;
}
